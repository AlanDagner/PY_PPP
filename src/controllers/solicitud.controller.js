...
export const listarSolicitud = async (req,res)=>{
    try {
        const response = await pool.query('SELECT e.e_nombre, es.codigo, s.estado_solicitud, es.ciclo, p.nombre, p.ape_paterno, p.ape_materno FROM solicitud s, empresa e, estudiante es, persona p  where s.id_empresa = e.id_empresa AND s.id_estudiante = es.id_estudiante and es.id_persona = p.id_persona');
